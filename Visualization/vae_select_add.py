...
def parse_args():
    parser = argparse.ArgumentParser(description="使用Wan VAE对两张图像进行潜空间Add融合")
    ...
    parser.add_argument("--input_path_b", type=str, default=default_path_b,
                        help="输入图像 B 路径")
    parser.add_argument("--output_path", type=str, default="fused_result_add.png",
                        help="输出融合图像路径")
...
def process_fusion(vae, tensor_a, tensor_b):
    """核心融合逻辑：Encode -> Add Fusion -> Decode"""
    with torch.no_grad():
        # 1. 编码 (Encode)
        # 使用 mode() 而不是 sample()。
        ...
        # 2. 融合 (Fusion) - Element-wise Add
        # 逻辑：将两个特征图对应位置的值相加
        logger.info("正在执行 Add 融合...")
        
        # 将两个潜空间特征相加
        latent_fused = latent_a + latent_b
