...
class Diffusion(nn.Module):
    ...
    def p_losses(self, model, x_start, t, cond):
        noise = torch.randn_like(x_start) #生成与张量 x_start 形状相同的高斯噪声（也称为正态分布噪声）
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)#对 x_start 进行噪声污染，生成 noisy 版本的 x
        model_output = model(x_noisy, t, cond) #预测噪声。
        ...
        loss_simple = self.get_loss(model_output, target, mean=False).mean() #计算损失
        return loss_simple
