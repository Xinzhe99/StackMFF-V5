...
class ControlLDM(nn.Module):
    ...
    def __init__(
        self, unet_cfg, clip_cfg, controlnet_cfg, latent_scale_factor, vae_cfg=None
    ):
        super().__init__()
        self.unet = ControlledUnetModel(**unet_cfg)
        # VAE 是可选的，如果外部提供了 VAE（如 FusionEncoder），则不需要创建
    ...
    def load_pretrained_sd(
        ...
        # 冻结已加载的模块
        modules_to_freeze = [self.clip, self.unet]
        if self.vae is not None:
            modules_to_freeze.append(self.vae)
