...
class Pipeline:
    ...
    def apply_cldm(
        ...
        if self.cond_fn:
            self.cond_fn.load_target(cond_img * 2 - 1) #将图像像素值从 0, 1 范围转换到 -1, 1 范围
        ...
        parameterization = self.diffusion.parameterization #此处用的是预测噪声
        ...
        with VRAMPeakMonitor("sampling"):
            z = sampler.sample(
                model=self.cldm,#调用CLDM模型
                device=self.device,
                steps=steps,#外部传入的采样步数
                x_size=(bs, 4, h2, w2),
                cond=cond,
                uncond=uncond,
                cfg_scale=cfg_scale,
...
class StackMFFV5Pipeline(Pipeline):
    def apply_cleaner(
        self, lq: torch.Tensor, tiled: bool, tile_size: int, tile_stride: int
    ) -> torch.Tensor:
        ...
        if tiled and (lq.size(2) < tile_size or lq.size(3) < tile_size):
            print("[StackMFFV5]: the input size is tiny and unnecessary to tile.")
            tiled = False
