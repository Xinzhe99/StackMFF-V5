...
class StackMFF_V5(nn.Module):
    def __init__(self, vae_model_id="Qwen/Qwen-Image-2512", vae_subfolder="vae"):
        ...
        self.depth_transformer = DepthTransformer(input_channels=16, embed_dim=256, num_heads=8, num_layers=4)
    ...
    def encode_one_step(self, x_with_time):
        """
        单个 batch 的编码步骤，方便用于 DataParallel 或 chunking
        Args:
            x_with_time: [B, C, 1, H, W]
        Returns:
    ...
    def forward(self, x, chunk_size=None):
        ...
        if chunk_size is not None and chunk_size > 0:
            latents_list = []
            total = x_with_time.shape[0]
            for i in range(0, total, chunk_size):
                end = min(i + chunk_size, total)
                batch_x = x_with_time[i:end]
                # 编码
            ...
            latents = self.encode_one_step(x_with_time)
        ...
        # 3. VAE Decode
        fused_latent_with_time = fused_latent.unsqueeze(2).to(dtype=self.vae_dtype)
        fused_output = self.vae.decode(fused_latent_with_time).sample
        fused_output = fused_output.squeeze(2).to(dtype=x.dtype)
...
if __name__ == "__main__":
    ...
    # 读取真实的测试图像
    img_path_a = "/home/ot/Students/xxz/projects_mff/StackMFF V5/Visualization/test_pair/test_A.jpg"
    img_path_b = "/home/ot/Students/xxz/projects_mff/StackMFF V5/Visualization/test_pair/test_B.jpg"
