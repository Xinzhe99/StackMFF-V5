...
from image_datasets.control_dataset_stackmff import loader, image_resize
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=r'/home/ot/Students/xxz/projects_mff/StackMFFV5/flymyai-lora-trainer/train_configs/train_lora_qwen_edit-2511-stackmff.yaml',
        help="path to config",
    )
...
def real_fusion_interface(latents, fusion_model):
    ...
    Input: latents of shape (Stack_Size, C, T, H, W) where T=1
           OR (Stack_Size, C, H, W) if compressed.
    Output: fused latent of shape (C, 1, H, W) to match Qwen Training Pipeline expects (C, T, H, W).
    """
    # 1. Prepare Input for DepthTransformer: [B, N, C, H, W]
    # Current 'latents' is either [Stack, C, 1, H, W] or [Stack, C, H, W]
    
    # Add Batch dimension -> [1, Stack, ...]
    stack_input = latents.unsqueeze(0) #-> [1, Stack, C, T, H, W]
    
    # If we have the extra Time dimension (T=1) from VAE, squeeze it out.
    # Check for 6 dims: [1, Stack, C, T, H, W]
    if len(stack_input.shape) == 6 and stack_input.shape[3] == 1:
         stack_input = stack_input.squeeze(3) # -> [1, Stack, C, H, W]
    
    # 2. Perform Fusion
    # DepthTransformer input: [B, N, C, H, W]
    # DepthTransformer output: [B, C, H, W]
    with torch.no_grad():
        fused_output = fusion_model(stack_input) # -> [1, C, H, W]
    
    # 3. Restore Time Dimension for Dataset/Training Loop
    # The training loop expects [B, C, T, H, W] (after batch collation)
    # The dataset expects [C, T, H, W] (single item)
    # We maintain T=1 compatibility.
    
    # [1, C, H, W] -(squeeze B)-> [C, H, W] -(unsqueeze T)-> [C, 1, H, W]
    return fused_output.squeeze(0).unsqueeze(1)
...
def main():
    ...
    if hasattr(args, 'fusion_model_path') and args.fusion_model_path:
        ...
        # Handle prefix if saved with 'depth_transformer.'
        new_state_dict = {}
    ...
    if args.precompute_text_embeddings:
        with torch.no_grad():
            ...
            for img_name in tqdm([i for i in os.listdir(args.data_config.img_dir) if ".png" in i or '.jpg' in i]):
                img_path = os.path.join(args.data_config.img_dir, img_name)
                # Ensure txt filename matches what dataset expects (usually simply replacing extension)
                # Dataset logic: txt_path = self.images[idx].split('.')[0] + '.' + self.caption_type
                # Here self.images are from img_dir.
                # So we should base the key on img_name which is from control_dir (which should match img_dir base names).
                ...
                # Use simple formatting for key to match dataset exactly
    ...
    if args.precompute_image_embeddings:
        ...
        with torch.no_grad():
            # Assume filenames in img_dir match folder names in control_dir (minus extension)
            target_images = [i for i in os.listdir(args.data_config.img_dir) if ".png" in i or ".jpg" in i]
            for img_name in tqdm(target_images, desc="Precomputing control stack embeddings"):
                base_name = img_name.rsplit('.', 1)[0]
                control_stack_dir = os.path.join(args.data_config.control_dir, base_name)
                
                if not os.path.isdir(control_stack_dir):
                    print(f"Warning: Control stack directory not found for {img_name}: {control_stack_dir}")
                    continue

                stack_files = sorted([f for f in os.listdir(control_stack_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))])
                
                if not stack_files:
                    print(f"Warning: No images found in {control_stack_dir}")
                    continue

                stack_latents = []
                for stack_img_name in stack_files:
                    img = Image.open(os.path.join(control_stack_dir, stack_img_name)).convert('RGB')
                    calculated_width, calculated_height, _ = calculate_dimensions(1024 * 1024, img.size[0] / img.size[1])
                    img = text_encoding_pipeline.image_processor.resize(img, calculated_height, calculated_width)
                    ...
                    latent = vae.encode(pixel_values).latent_dist.sample().to('cpu')[0]
                    stack_latents.append(latent)
                
                # Stack all latents: (Stack_Size, C, H, W)
                stack_latents_tensor = torch.stack(stack_latents)

                # Move tensor to the same device as the fusion model
                stack_latents_tensor = stack_latents_tensor.to(accelerator.device, dtype=weight_dtype)
                
                # Fuse the stack
                fused_latent = real_fusion_interface(stack_latents_tensor, fusion_model)

                # Move back to CPU for saving/caching to save GPU memory
                fused_latent = fused_latent.to('cpu')

                if args.save_cache_on_disk:
                    torch.save(fused_latent, os.path.join(img_cache_dir, img_name + '.pt')) # Save using target image name
                    del stack_latents
                    del stack_latents_tensor
                    del fused_latent
                else:
                    cached_image_embeddings_control[img_name] = fused_latent # Save using target image name
        vae.to('cpu')
        fusion_model.to('cpu')
    ...
    logger.info(f"Memory before loading transformer: {torch.cuda.memory_allocated()/1024**3:.2f} GB allocated, {torch.cuda.memory_reserved()/1024**3:.2f} GB reserved")
    ...
    if args.quantize:
        ...
        for block in tqdm(all_blocks, desc="Quantizing blocks"):
            ...
            quantize(block, weights=qint8, activations=qint8)
            freeze(block)
            block.to('cpu')
    ...
    # Disable gradient checkpointing for a moment to see if it fixes the patch
    # Gradient checkpointing often re-wraps the forward pass and can bypass monkey patches or interact with torch.compile
    # flux_transformer.disable_gradient_checkpointing()
